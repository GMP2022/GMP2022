...
void Preprocess::load_data(const std::string& path)
{
	...
	if (!is) {
		std::cerr << "Fail to find the data file" << std::endl;
		exit(0);
	}
	int header[3] = { 0 };
	...
	float* array_ = new float[(size_t)header[2] * header[1]];
...
{
	benchmark.N = 1000, benchmark.num = 100;
...
Preprocess::~Preprocess()
{
	...
	clear_2d_array(InnerProduct, 201);
...
Partition::Partition(int m_, Preprocess& prep)
{
	num_chunk = m_;
